...
2. 在左侧导航栏，选择**系统管理** > **集群管理**，查看[各组件的运行状态](../user-guide/manage-system/manage-cluster.md)，主要检查：
   - TapData 服务相关进程：Tapdata管理器、Tapdata引擎、Tapdata API服务器是否运行正常。
   - 检查 CPU 和内存使用率是否处于健康水平（低于70%）

3. 单击左侧导航栏的**数据复制**或**数据转换**，查看任务列表，主要检查各任务的状态是否均为运行中。
   此外，您还可以单击任务名称，查看任务详细[监控信息](../user-guide/copy-data/monitor-task.md)，确认任务运行正常，同步延迟是否在可接受范围内，任务 QPS 是否大于0。
   如果任务出现异常，请跟随下述步骤排查：
   * **检查任务报错信息**：如果遇到任务异常，您可以通过任务的监控页面的底部查看相关日志信息，根据日志提示进行故障排除。更多介绍，见[任务故障排查](../administration/troubleshooting/README.md)。
   * **检查任务关联的数据库连接**：单击左侧的**连接管理**，找到对应的数据源连接，单击其**测试连接**按钮以验证连通性，如遇异常，请根据提示进行故障排除（如密码错误）。
   * **检查任务增量延迟**：如果 QPS 突增持续 30 分钟以上，可能是源端批量操作或业务高峰引起，请评估任务是否需扩容；如果目标库无增量写入或写入耗时异常，请检查 CDC 的前提准备是否正确设置，以 MySQL 数据库为例，需确认是否开启了 binlog 并设置为 ROW 模式，具体设置可参考对应的[创建连接文档](../prerequisites/README.md)。此外，任务日志中如出现主键冲突，需确认是否有配置改动。

如经过上述流程排查，任务仍异常，请[联系 Tapdata 支持](../appendix/support.md)。
...
- 升级 TapData 服务前，推荐通过 `mongodump` 命令或其他方式，对 TapData 依赖的元数据库进行备份。
...
TapData 提供多层次的任务调度机制，包括默认的基于任务数量的调度策略、灵活的 Flow Engine（流引擎） 分组调度，以及基于标签的启动优先级控制。这些机制确保任务高效、均衡地运行在合适的 Flow Engine 上，避免资源浪费和单点过载。

**使用建议**：日常场景优先使用基础调度机制，简单高效；对性能或隔离有要求的任务，推荐启用 Flow Engine 分组调度；大批量启动任务时，利用标签控制顺序，避免重要任务被阻塞。


### 基础调度机制（默认）

系统默认采用基于任务数量的均衡策略，自动将任务分配到最优 Flow Engine：

- **启动调度**：任务启动时，系统查询所有存活 Flow Engine 的当前运行任务数量，优先调度到任务数量最少的 Flow Engine。
- **故障转移**：Flow Engine 心跳超时或任务接管失败时，立即触发重新调度，将受影响任务转移到当前任务数量最少的存活 Flow Engine。

此机制适用于大多数场景，无需额外配置，即可实现高效资源利用。

### Flow Engine 分组调度

为满足不同业务对性能、隔离或专用资源的需求，TapData 支持 Flow Engine 分组调度。

**配置步骤**：  
进入[集群管理](../user-guide/manage-system/manage-cluster.md)页面，切换到组件视图，根据机器性能、业务线等维度创建分组（如“高性能组”或“业务A专用组”），并将相应 Flow Engine 加入。

**调度规则**：
- 在创建连接或任务时，可选择特定分组进行调度。
- 系统仅在选定分组内的 Flow Engine 中调度任务。
- 分组内仍遵循任务数均衡原则（优先选择任务数量最少的 Flow Engine）。
- 如果分组内所有 Flow Engine 不可用，任务进入等待状态，并触发[告警通知](../user-guide/other-settings/notification.md)，便于管理员及时介入。
...
**配置步骤**：  
在[任务管理](../user-guide/copy-data/manage-task.md)页面，为任务设置标签（优先级范围 P1~P6）。
...
:::tip
标签调度仅控制任务的启动顺序，不影响运行时的 Flow Engine 分配。
:::
...
您可以在多台机器上[部署 TapData](production-deploy/install-tapdata-ha.md) 以实现高可用，某个节点出现异常时，其余节点可继续提供服务，进入增量的任务会断点续传，任务可自动均衡调度分配。
...
1. 登录 TapData 所依赖的 MongoDB 数据库。

2. 进入 `tapdata` 数据库，执行以下命令：

   ```sql
   db.Events.find().sort({_id: -1})
   ```

   此命令将按时间倒序查看最新的告警事件记录。

3. **分析关键字段**：

   - 如果记录中包含 `successful`，表示邮件已发送成功。
   - 如果记录中包含 `failed`，表示邮件发送失败，此时可以查看 `fail_message` 字段的值，分析具体失败原因。
...
* [数据复制](../user-guide/copy-data/create-task.md)/[数据转换](../user-guide/data-development/create-task.md)任务
