...
TapData 提供多层次的任务调度机制，包括默认的智能负载均衡、灵活的 Agent 分组调度，以及基于标签的启动优先级控制。这些机制确保任务高效、均衡地运行在合适的 Agent 上，避免资源浪费和单点过载。

**使用建议**：日常场景优先使用默认负载均衡，简单高效；对性能或隔离有要求的任务，推荐启用 Agent 分组调度；大批量启动任务时，利用标签控制顺序，避免重要任务被阻塞。


### 基础调度机制（默认）

系统默认采用基于任务数量的负载均衡策略，自动将任务分配到最优 Agent：

- **启动调度**：任务启动时，系统查询所有存活 Agent 的当前运行任务数量，优先调度到任务数量最少的 Agent。
- **故障转移**：Agent 心跳超时或任务接管失败时，立即触发重新调度，将受影响任务转移到当前任务数量最少的存活 Agent。
- **动态均衡**：系统持续监控 Agent 负载，实现任务自动再平衡，保持整体分布均匀。

此机制适用于大多数场景，无需额外配置，即可实现高效资源利用。

### Agent 分组调度

为满足不同业务对性能、隔离或专用资源的需求，TapData 支持 Agent 分组调度。

**配置步骤**：  
进入[集群管理](../user-guide/manage-system/manage-cluster.md)页面，切换到组件视图，根据机器性能、业务线等维度创建分组（如“高性能组”或“业务A专用组”），并将相应 Agent 加入。

**调度规则**：
- 在创建连接或任务时，可选择特定分组进行调度。
- 系统仅在选定分组内的 Agent 中调度任务。
- 分组内仍遵循负载均衡原则（优先选择任务数量最少的 Agent）。
- 如果分组内所有 Agent 不可用，任务进入等待状态，并触发[告警通知](../user-guide/other-settings/notification.md)，便于管理员及时介入。
...
- 无标签任务：采用默认顺序（通常后于有标签任务）。

:::tip
标签调度仅控制任务的启动顺序，不影响运行时的 Agent 分配。
:::
