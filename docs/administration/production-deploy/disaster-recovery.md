...
- 引擎故障（TapData Engine）
  - 机制：任务心跳超时后自动切换到健康引擎，默认超时为 5 分钟。
  - 影响：前端几乎无感知；引擎与 TM 的交互为幂等，请求重试不会造成副作用。
  - 建议：根据业务 SLA 调整任务心跳检测间隔与超时阈值，必要时缩短检测窗口。

- MongoDB 故障
  - 机制：副本集自动选举，客户端驱动秒级重连至新主节点。
  - 影响：对 TM/Engine 前端几乎无感知；在 `w:majority` 写入场景下，主节点切换期间可能出现短暂等待。
  - 建议：保持统一的副本集连接字符串并启用驱动重试；关注 `wtimeout` 指标与复制延迟。

- TM 故障（TapData Management）
  - 机制：前端通过 Nginx/HAProxy 等负载均衡进行健康检查并自动摘除异常实例；引擎默认通过轮询选择正常的 TM。
  - 影响：几乎无感知；引擎请求幂等，任务执行不受影响。
  - 建议：为 TM 实例配置健康检查、熔断与限流策略，并监控可用实例占比。

:::tip

上述为单组件故障处理，通常不触发跨区域灾备切换。发生区域性故障时请按第 3 章“灾难恢复操作流程”执行。
...
- **组件级故障参考 RTO（不触发跨区 DR）**：
  - MongoDB 副本集主从切换：秒级（客户端驱动自动重连）
  - TM 实例故障：秒级（负载均衡与引擎轮询处理）
  - 单引擎故障：≤ 5 分钟（受任务心跳超时默认值影响）
...
- **业务逻辑**：数据同步任务状态、复制延迟监控，每 1 分钟（结合任务心跳默认 5 分钟）
...
- 引擎任务心跳超时（默认 5 分钟）→ 自动切换并记录事件；持续超时 → Critical 告警
- TM 实例不可用由负载均衡自动摘除；可用 TM 少于 50% → Critical 告警
