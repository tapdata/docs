...
Tapdata Cloud 支持读取 Local（本地）、FTP、SFTP、SMB、S3FS 或 OSS 上的文件，支持的文件类型包括 [CSV](../../beta/file/csv.md)、[EXCEL](../../beta/file/excel.md)、JSON 和 [XML](xml.md)，满足多样化的数据流转需求。
...
- Tapdata Cloud 默认每隔 1 分钟检测 JSON 文件是否发生变更（如新增/修改文件），如发生变更则将涉及的文件全量新增后更新条件字段以完成修改，暂不支持同步删除文件或数据。
