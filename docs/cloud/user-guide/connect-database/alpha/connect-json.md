...
JSON（JavaScript Object Notation）是一种轻量级的数据交换格式，易于阅读和编写，可以在多种语言之间进行数据交换。Tapdata Cloud 支持读取本地、FTP、SFTP、SMB、S3FS 或 OSS 上的 JSON 文件，满足多样化的数据流转需求。本文介绍如何在 Tapdata Cloud 上连接 JSON 数据源。
...
5. 在跳转到的页面，根据下述说明填写 JSON 的连接信息。
   ...
   * **文件协议**：根据 JSON 文件存放的位置选择下述协议，本文以 **S3FS**（Amazon S3 存储桶） 为例演示操作流程。 
     :::tip
     关于如何获取各协议所需填写的信息，见 [JSON/XML 数据源准备工作](../../../prerequisites/config-database/alpha/json-and-xml.md)。     
     :::     
