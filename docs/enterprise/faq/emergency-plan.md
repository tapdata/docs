...
Tapdata 支持单机或高可用部署，如您部署在生产环境中，推荐采用[高可用部署](../quick-start/install/install-tapdata-ha.md)方式。
...
在[高可用环境](../quick-start/install/install-tapdata-ha.md)下，单个引擎异常后，其任务预计在 10 分钟内逐渐被其他引擎接管，最小化任务影响，同时会在后台尝试自动恢复异常的引擎。
