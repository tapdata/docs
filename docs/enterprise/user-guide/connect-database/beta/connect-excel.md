...
5. 在跳转到的页面，根据下述说明填写 Excel 的连接信息。
   ...
   * **文件协议**：根据 Excel 文件存放的位置选择下述协议，本文以 **FTP** 为例演示操作流程。
     ...
     * **S3FS**（遵循S3协议文件系统）：文件存放在 S3FS 服务器上，选择此项后，您还需要填写 Accesskey、Secretkey、终端、桶和文件路径等信息。
     * **OSS**（对象存储服务）：文件存放在 OSS 对象存储服务上（如阿里云对象存储），选择此项后，您还需要填写云账户的 Accesskey、Secretkey、终端、桶和文件路径等信息。
