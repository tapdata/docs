...
- **消息推送保障**：将 Kafka 作为源端消费时，仍需关注重复消费风险，并在目标端通过幂等设计避免重复数据。将 Kafka-Enhanced 作为目标端写入时，支持事务写入，可配合任务的精确一次写入能力提升写入一致性。
- **消费模式限制**：消费线程采用不同的消费组编号，请注意对消费并发度的影响。
- **安全认证**：支持免认证和 SASL 用户名密码认证，可按 Kafka 实例的认证方式选择 PLAIN、SCRAM-SHA-256 或 SCRAM-SHA-512。
