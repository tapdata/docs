...
- 部署裸日志服务可用于解析 Db2 的增量数据变更，在该服务部署完毕后，需要为其配置与 Db2 数据库相同的国家码语言码。如需使用裸日志高可用能力，请在多个节点上部署裸日志服务，并确保各节点的语言码和国家码配置一致。
  ```bash
    # 语言码 UTF-8
...
- 考虑到连接测试会自动触发裸日志服务的 LRI（Log Record Identifier）缓存，在多节点集群场景下，建议您通过多次连接测试确保每个节点都被命中并缓存 LRI，从而避免任务在首次进入 CDC 或发生节点切换时因重新查找 LRI 而产生额外延迟。
...
3. 如需增量数据同步，您需要为[联系 TapData 团队](../../appendix/support.md)，获取并部署裸日志采集服务。
...
5. 在跳转到的页面，根据下述说明填写 Db2 的连接信息。
   ...
   * **连接信息设置**
     ...
     * **高可用**：开启后，可通过**裸日志集群**添加多个裸日志服务节点，提升 Db2 裸日志采集链路的可用性。
     * **裸日志集群**：单击**添加**，依次填写多个裸日志服务的地址和端口。CDC 任务启动时，TapData 将按本地优先、远程次之的规则选择裸日志服务；同一优先级下存在多个可用服务时，将随机分配。
   ...
   * **SSL 设置**：如 Db2 服务端已启用 TLS/SSL，可开启**使用 SSL**，并按数据库侧证书配置上传 **CA 文件**、**客户端证书文件**、**客户端密钥文件**，必要时填写**客户端密钥密码**。
