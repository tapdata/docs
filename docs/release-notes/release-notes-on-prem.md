...
- 新增[聚合计算节点](../user-guide/data-development/aggregate-computing-node.md)（Aggregate），支持通过可视化界面进行数据分组、汇总与派生计算，满足前置计算与统一业务统计口径的需求

### 功能优化

- 优化任务列表的指标展示，新增指标说明、延迟状态及更多关键运行信息
- 优化 Paimon 目标端配置，支持导入/导出建表自定义设置
- 优化 Db2 裸日志（Raw Log）解析能力，支持高可用（HA）架构
- 优化 Kafka 写入逻辑，支持按更新条件分桶，并允许将主键更新事件拆分为 Delete 和 Insert 操作
- 优化自定义节点能力，支持单次处理返回多条数据记录
- 优化内置 MongoDB 的连接超时与高可用重试策略，提升系统稳定性
- 支持在 JS 节点中模拟（Mock）输入数据进行快速调试

### 缺陷修复

- 修复数据复制或转换任务中，偶现无法显示运行日志的问题
- 修复中间库连接异常时，可能导致管理节点（TM）意外退出的问题
- 修复实时数据中心未能正确显示 AS400 数据源行数和存储大小的问题
- 修复任务缓存页面无法正常显示缓存信息的问题
- 修复重建缓存时因 `_id` 字段更新导致的任务报错问题
- 修复共享缓存可能读取到过期数据从而影响数据一致性的问题
- 修复主从合并平铺场景下，子表关联键在主表中发生变更时未能正确清理的问题
- 修复主从合并任务中出现大量无效警告日志的问题
- 修复数据校验任务中推荐的关联键顺序不正确的问题
- 修复无增量数据时重启任务可能误触发延迟告警的问题
- 修复增量同步特定时间类型数据时报空指针异常的问题
- 修复特定场景下 JSON 数据解析报错的问题
- 修复达梦数据库同步至 Doris 时的 DDL 处理异常问题
- 修复 MySQL 同步负数时间格式数据时报错的问题
- 修复 MongoDB 与 MySQL 进行数据校验时，数组和对象类型对比失败的问题
- 修复 PostgreSQL 分区表同步至 SQL Server 时任务报错的问题
- 修复 SQL Server 开启 Database Log 后未能正确输出日志的问题
- 修复 SQL Server 同步至 SQL Server 时，如果任务出错停止，未能将报错信息正确输出到前端页面日志的问题
...
- [Paimon 连接器](../prerequisites/warehouses-and-lake/paimon.md#节点高级特性)新增按表配置分区开关，并允许自定义分区键、分桶、压缩格式等属性，提升数据管理的灵活性
...
- 修复无法正确解析 Kafka 中 Debezium 结构的 AVRO 格式数据的问题
- 修复 MariaDB 同步至 Doris 任务在全量完成后 CDC 阶段失败的问题
- 修复 MySQL 任务中因字段重命名处理器异常导致同步失败的问题
- 修复 MongoDB 同步至 MySQL 时，`BSONRegExp` 类型数据导致任务运行错误的问题
- 修复 PostgreSQL CDC 任务在引擎重启后，因 WAL 日志已被清理导致任务无法恢复的问题
- 修复 TDSQL 开启全量断点续传功能后导致任务报错的问题
- 修复 API Server 审计日志在请求成功（Code 200）时仍包含异常信息的问题
- 修复在创建 API 时，未进行重名校验的问题
- 修复发布的 API 接口无法正确处理参数过滤的问题
- 修复基于 MySQL 发布 API 失败的问题
- 修复同步任务中删除 `_id` 字段后，生成的 API 无法通过主键查询数据的问题
- 修复使用 MongoDB 8.0 作为中间存储时任务启动失败的问题
- 修复开启多表并发读取后，全量数据批量写入性能下降的问题
- 修复修改连接的运行引擎标签（Tag）后，任务启动时误报标签不存在的问题
- 修复连接管理页面从编辑页返回列表页时，查询筛选状态丢失的问题
- 修复定时校验任务异常循环触发二次校验的问题
...
- MongoDB 作为源场景下，支持基于队列/文档大小等智能识别内存超限风险并提示调整批次中的文档条目数，保障任务稳定运行

### 功能优化

- 增强 JS 节点对存储过程调用的支持，完善入参处理及多结果集返回等复杂场景的兼容性
- API 服务现支持直接展示 Token 获取代码示例，简化调用流程
- 优化 Paimon 连接器读取逻辑，提升数据同步效率
- 支持对任务标签进行排序，提升筛选体验
- 优化 License 更新流程及节点数计算逻辑

### 缺陷修复

- 修复任务监控页面主从合并节点字段名显示不全且无法复制的问题
- 修复主从合并任务中，若存在禁用表会导致所有表无法重建缓存的问题
- 修复 Db2 同步至 PostgreSQL 时，Double 类型字段映射精度不足导致数据截断的问题
- 修复 Db2 读取含特殊字符数据时报字符集编码错误的问题
- 修复 PostgreSQL 分区表在 CDC 阶段可能出现的权限校验警告问题
- 修复 AS400 GRAPHIC 类型字段未能自动转换为目标库兼容类型的问题
...
- [系统设置](../user-guide/other-settings/system-settings.md)新增环境级可视化标识能力，开启后可在顶部展示环境标签，快速识别部署环境，降低误操作风险
- [模型对比功能](../user-guide/compare-data-model.md)新增主键一致性检查，适用于手动建表场景，提前发现主键不一致并提示修正
- [任务监控功能](../user-guide/copy-data/monitor-task.md)新增按时间范围展示 P99/P95 延迟指标，更直观反映任务性能与尾延迟


### 功能优化

- 统一调整所有数据源名称为标准命名，提升整体配置的一致性与可识别性
- 优化 API 发布交互，禁用 Array 字段下嵌套字段的点击选中与重命名操作，避免误配置带来的接口定义风险
- 复制任务在运行过程中，支持一次性汇总并报告所有存在异常的表，提升异常定位与问题排查效率


### 问题修复

- 修复在动态新增表与多表合并场景下，新增表后任务异常停止的问题
