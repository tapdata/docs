...
- 发布 Db2 for i 连接器，支持主流数据源的同步与写入能力
- [增量数据校验功能](../user-guide/incremental-check.md)支持批量手动校验与修复，并支持设置忽略指定表不执行校验，满足定制化需求
- [主从合并节点](../user-guide/data-development/process-node.md#主从合并)支持子表缓存的重建能力，用于修复缓存不一致问题
- 复制任务新增支持 MongoDB 作为目标端的精确一次写能力，确保写入幂等性与一致性
- Sybase 连接器新增支持精确一次写能力
- 任务日志新增支持输出 DDL 语句，提升排查与审计可读性
- 支持对数据源增量读取批次进行自动调整，更好平衡吞吐性能与稳定性


### 功能优化

- API 审计日志详情优化，新增记录数据库响应时间
- API 性能优化，提升整体请求处理效率
