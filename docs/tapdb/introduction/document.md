...
- _id字段可以包含任何BSON 数据类型的值，数组、正则表达式或未定义除外。
  ...
  >警告:为了确保复制功能正常，请勿在 _id 字段中存储 BSON 正则表达式类型的值。
...
> 注意
> 
> 大多数 TapDB 驱动程序客户端将包含_id字段并生成ObjectId ，然后再将插入操作发送到 TapDB；但是，如果客户端发送的文档没有_id字段，则tapdb将添加_id字段并生成ObjectId 。
