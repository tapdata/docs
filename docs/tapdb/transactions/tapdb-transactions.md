...
与单文档写入操作相比，分布式事务通常具有更高的性能开销，因此应优先考虑有效的模式设计。非规范化的数据模型（嵌入式文档和数组）在很多情况下仍是最佳选择。适当的数据建模将最大限度地减少对分布式事务的需求。

有关其他事务使用注意事项（如运行时间限制和 oplog 大小限制），另请参阅[生产注意事项](../administration/production-notes.md)
...
- 有关事务中不支持的操作列表，请参阅[限制操作](#limitation)。
...
<span id="limitation">事务</span>中不允许执行以下操作：
