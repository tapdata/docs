...
数据转换任务启动后，页面将自动跳转至任务监控页面，通过该页面可观察任务运行细节，包括 Agent 状态、数据同步状态、任务进度、告警设置等信息。您也可以先在任务列表页面查看任务整体运行概况，再进入单个任务查看详情。
...
在**数据转换**任务列表页面，您可以通过任务状态、任务里程碑、增量延迟等指标快速判断任务是否正常运行。常见字段说明如下：

| 字段 | 说明 |
| --- | --- |
| **任务名称** | 任务的显示名称。名称下方会展示任务同步类型，如全量、增量或全量 + 增量。 |
| **任务状态** | 任务当前所处状态，例如编辑中、运行中、已停止或运行失败。 |
| **任务里程碑** | 展示任务当前执行阶段，帮助判断任务处于初始化、全量同步、增量同步或其他处理阶段。 |
| **增量延迟** | 源端事件产生到任务处理完成之间的时间差，用于判断增量同步是否存在积压。 |
| **增量时间点** | 任务当前处理到的增量数据事件时间点。该时间点越接近当前时间，说明增量处理越及时。 |
| **上次运行时间** | 任务最近一次启动运行的时间。 |
| **创建时间** | 任务创建时间。 |
| **更新时间** | 任务配置或状态最近一次更新的时间。 |
| **可用操作** | 支持对任务执行启动、编辑、监控、重置、复制、删除等操作，具体可用项与任务状态有关。 |

如果需要查看节点级指标、运行日志、告警或任务设置，可单击目标任务右侧的**监控**进入任务详情页。
...
* RPS：任务平均每秒处理的输入事件数和输出事件数，用于衡量数据传输速度和性能，支持基于事件数量或大小的维度来查看。
...
单击该错误码，可在弹出的对话框中详细了解详细的原因及解决方案，更多信息，见[任务错误码及解决方案](../../administration/troubleshooting/error-code.md)。
