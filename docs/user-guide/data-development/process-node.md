...
- 参与合并的表必须包含一个或多个可组成逻辑唯一标识的字段，以确保数据的唯一性。
...
此外，当发现数据不一致或不符合预期时，可停止并编辑该任务，单击主从合并节点，在缓存管理中，对需要重新同步的子表缓存执行重建操作。
...
连接节点主要用来进行表之间的连接设置，支持**左连接**，选择相应字段进行关联即可进行两个表的合并。

![连接处理节点](../../images/join_processing_node.png)

:::tip

连接的源端必须是表节点，且在连接节点进行数据处理时，系统不会自动为目标表创建主键。如需设置主键，可通过 JS 处理节点手动指定。

:::

如上图所示，我们通过连接节点，将两个 MySQL 数据库的表合并，然后将其同步到 Oracle 数据库中，假设两个 MySQL 表的结构和数据如下：
...
任务启动后，数据经过实时 Join 并同步到 Oracle，其表结构和数据如下：
...
Python 处理节点当前不支持使用自定义依赖包，且暂不适用于在 Windows 环境下部署的引擎。
