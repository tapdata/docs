...
4. 在弹出的对话框中，基于业务需求完成增量数据校验配置。
   ...
   - **表过滤**：支持配置需要校验的表名，默认校验所有表，可选择包含指定表或排除指定表，您也可以通过正则表达式筛选需要校验的表，更好满足定制化需求。
   - **时间精度**：支持严格匹配、四舍五入和截断，默认严格匹配，即要求源端和目标端的时间字段值完全一致。四舍五入和截断则允许一定的时间误差，适用于对时间精度要求不高的场景。
   -  **队列缓存大小**：默认为自动，即系统根据任务的并发度和校验频率动态调整缓存大小，手动调大需要注意系统资源占用。
   - **校验无主键表**：默认不校验无主键表，即仅校验包含主键的表。如果源端或目标端有无主键表，建议先配置好主键，或手动开启此选项。
...
5. （可选）完成配置并启动任务后，在任务监控页面，可查看到增量校验的执行记录，包括抽样次数、差异条数等。
   ...
   :::tip
   发现差异时，可点击右上角“生成自定义修复 SQL”，一键导出差异 SQL 供 DBA 评估与精准修复。此外，还支持批量手动修复差异数据，确保数据一致性。
   :::