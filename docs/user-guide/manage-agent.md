...
<TabItem value="阿里云计算巢">

1. 登录阿里云并进入[计算巢控制台](https://computenest.console.aliyun.com/)。

2. 找到您的计算巢实例并单击其 ID，进入概览页面。

3. 单击**资源**页签，找到 Agent 安装到的 ECS 示例（名称包含 **tapdata**），单击其右侧的**远程连接**。

    ![](https://docs.tapdata.io/img/login_compute_nest_cn.png)

4. 在跳转到的命令窗口中，执行 `cd /data/tapdata` 进入 Agent 安装目录，然后选择执行下述命令：
    - 查看命令帮助：`./tapdata help`
    - 查看 Agent 状态：`./tapdata status`
    - 启动 Agent：`./tapdata start`
    - 停止 Agent：`./tapdata stop`
