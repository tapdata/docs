...
* **[高效的数据流开发工具套件](developer-guide/README.md)**

  为开发者提供实时数据处理工具套件 TapData Shell（也称 TapShell），支持全代码化任务定义，通过交互式命令行或 Python 程序的方式实现任务调度、数据清洗与处理，特别适合复杂业务和脚本化管理场景，大幅提升开发效率与灵活性。
