...
- **管理端和引擎之间断网**：
  云版不会影响任务运行，网络恢复后自动继续；企业版短时间断网没影响，但如果超过 10 分钟，任务会自动转移至其他可用的引擎进行执行，全量任务会从头开始，增量任务会从断点继续。
- **引擎和数据源之间断网**：
  系统会在设置的[重试时间](../user-guide/other-settings/system-settings.md#task-setting)内自动尝试连接，恢复后任务继续运行；如果超时未恢复，任务会停止，网络恢复后全量任务从头开始，增量任务从断点继续。
...
更多数据库类型支持计划，敬请关注后续版本更新。



### 双向数据同步如何处理写入冲突？

当前双向同步不支持并发写冲突的自动处理机制。由于数据同步发生在事务之外，系统无法感知源端和目标端的事务上下文，也就无法识别或解决同步过程中产生的数据冲突。
因此，不建议在双向同步场景中同时修改两端的相同数据，需通过业务逻辑或操作规范避免双写冲突。



### 重置任务配置会清空已同步的表和数据吗？

不会，重置任务配置只会清除任务运行的状态信息（如增量时间点），不会清除已同步到目标库的表和数据。
